...
# Directories set up
export DATA_ROOT=/net/me/merkur2/vystadial/asr-mixer/cs-super  # expects subdirectories train + $TEST_SETS
export WORK=`pwd`/model_cs_super_s2k_g50k
export EXP=$WORK/exp
export TGT_MODELS=exported/cs_super_s2k_g50k
export TGT_MODELS2=exported2/cs_super_s2k_g50k
...
export train_cmd="queue.pl -A css2kg50k -V -l mem_free=2G,h_vmem=4G -p -50 -q '`qselect | sort | egrep -v 'pandora5|pandora8|hyperion5|lucifer' | tr '\n' ',' | sed s/\,$//`"
export decode_cmd="queue.pl -A css2kg50k -V -l mem_free=4G,h_vmem=8G -p -50 -q '`qselect | sort | egrep -v 'pandora5|pandora8|hyperion5|lucifer' | tr '\n' ',' | sed s/\,$//`"
export njobs=100
